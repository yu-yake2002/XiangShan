...
object DataConfig {
  ...
  case class MtilexData() extends DataConfig("mtilex", log2Up(VecData().dataWidth) + 1 ) // 8
  case class FakeIntData() extends DataConfig("fakeint", 64)
  case class NoData() extends DataConfig("nodata", 0)

  def RegSrcDataSet   : Set[DataConfig] = Set(IntData(), FpData(), VecData(), V0Data(), VlData(), MtilexData())
  ...
  def MtilexRegSrcDataSet : Set[DataConfig] = Set(MtilexData())
