...
class WbFuBusyTableImp(override val wrapper: WbFuBusyTable)(implicit  p: Parameters, params: BackendParams) extends LazyModuleImp(wrapper) {
  ...
  private val mtilexAllWbConflictFlag = io.out.wbConflictRead.flatten.flatten.map(_.mtilexConflict)
  ...
  private val mtilexAllBusyTable = (intSchdBusyTable ++ fpSchdBusyTable ++ vfSchdBusyTable ++ memSchdBusyTable).flatten.map(_.mtilexWbBusyTable)
  ...
  private val mtilexAllDeqRespSet = (intSchdBusyTable ++ fpSchdBusyTable ++ vfSchdBusyTable ++ memSchdBusyTable).flatten.map(_.mtilexDeqRespSet)
  ...
  private val mtilexAllRespRead = (intRespRead ++ fpRespRead ++ vfRespRead ++ mfRespRead ++ memRespRead).flatten.map(_.mtilexWbBusyTable)
  ...
  private val mtilexAllBusyTableWithParms = mtilexAllBusyTable.zip(allExuParams).toSeq
  ...
  private val mtilexAllDeqRespSetWithParms = mtilexAllDeqRespSet.zip(allExuParams).toSeq
  ...
  private val mtilexWbLatencyMax = params.getMtilexWBExeGroup.map { case (portId, seq) => (portId, seq.map(_.mtilexLatencyValMax).max, seq.forall(_.mtilexLatencyCertain)) }
  ...
  private val mtilexWbBusyTable = mtilexWbLatencyMax.map { case (portId, latMax, latCertain) => (portId, OptionWrapper(latCertain, Wire(UInt((latMax + 1).W)))) }.toMap
  ...
  private val mtilexConflict = mtilexWbLatencyMax.map { case (portId, latMax, latCertain) => (portId, OptionWrapper(latCertain, Reg(Bool()))) }.toMap
  ...
  def hitWbPort[T <: Data](source: Option[T], p: ExeUnitParams, portId: Int, wbType: PregWB) = {
    wbType match {
      ...
      case MtilexWB(_, _) => p.wbPortConfigs.collectFirst { case x: MtilexWB => x.port }.getOrElse(-1) == portId && source.nonEmpty
  ...
  writeBusyTable(mtilexWbBusyTable, mtilexAllBusyTableWithParms, MtilexWB())
  //per wbPort conflict
  ...
  writeConflict(mtilexConflict, mtilexAllDeqRespSetWithParms, MtilexWB())
  ...
  readRes(mtilexAllRespRead, mtilexWbBusyTable, MtilexWB())
  ...
  readRes(mtilexAllWbConflictFlag, mtilexConflict, MtilexWB())
}
