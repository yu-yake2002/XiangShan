...
class DebugLSIO(implicit p: Parameters) extends XSBundle {
  val debugLsInfo = Vec(backendParams.LduCnt + backendParams.HyuCnt + backendParams.MlsCnt + backendParams.StaCnt + backendParams.HyuCnt + backendParams.MlsCnt, Output(new DebugLsInfoBundle))
}
