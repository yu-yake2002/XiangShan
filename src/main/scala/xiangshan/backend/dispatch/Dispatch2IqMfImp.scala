...
class Dispatch2IqMfImp(override val wrapper: Dispatch2Iq)(implicit p: Parameters, params: SchdBlockParams)
  extends Dispatch2IqImp(wrapper)
    with HasXSParameter {

  val mfIQValidNumVec = io.mfIQValidNumVec.get
  ...
  println(s"[Dispatch2IqMfImp] IQFuConfigs: ${IQFuConfigs.map(_.map(_.name))}")
  println(s"[Dispatch2IqMfImp] allFuConfigs: ${allFuConfigs.map(_.name)}")
  println(s"[Dispatch2IqMfImp] fuConfigMapIQ: ${fuConfigMapIQ.map(x => (x._1.name, x._2))}")
  println(s"[Dispatch2IqMfImp] fuConfigOnlyOneIQ: ${fuConfigOnlyOneIQ.map(_.name)}")
  println(s"[Dispatch2IqMfImp] fuConfigMultiIQ: ${fuConfigMultiIQ.map(_.name)}")
  ...
  val minIQSel = VecInit(fuConfigMultiIQ.map{ case fuConfig =>
    val thisFuValidNumSeq = VecInit(exeFuConfigs.zip(io.mfIQValidNumVec.get.flatten).filter{case (a,b) => a.contains(fuConfig)}.map(_._2))
  ...
  if(backendParams.debugEn){
    dontTouch(io.mfIQValidNumVec.get)
